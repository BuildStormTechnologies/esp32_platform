...
void DISP_shadowUpdateScreen(char *thingId, char *led, char *color)
{
    ...
    sprintf(dispBuff, "\n Thing : %s\n\n LED   : %s\n\n Color : %s", thingId, led, color);
    TFT_printWithColor(dispBuff, 0, y, msgColor, TFT_BLACK);
}
...
void DISP_welcomeScreen()
{
    ...
    TFT_printWithColor("Shadow Update Demo", CENTER, y + offset, TFT_LIGHTGREY, TFT_BLACK); //DEJAVU24_FONT // 20 char/line
...
void DISP_init()
{
    ...
    disp_varInit();
    disp_header("AWS SHADOW DEMO", "BuildStorm Technologies");
    DISP_welcomeScreen();
