...
// Set max width & height of the proportional font
//-----------------------------
static void getMaxWidthHeightForFont(Font *fontObj)
{
    uint16_t tempPtr = 4; // point at first char data
    uint8_t cc, cw, ch, cd, cy;

    fontObj->numchars = 0;
    fontObj->max_x_size = 0;

    cc = fontObj->font[tempPtr++];
    while (cc != 0xFF)
    {
        fontObj->numchars++;
        cy = fontObj->font[tempPtr++];
        cw = fontObj->font[tempPtr++];
        ch = fontObj->font[tempPtr++];
        tempPtr++;
        cd = fontObj->font[tempPtr++];
        cy += ch;
        if (cw > fontObj->max_x_size)
            fontObj->max_x_size = cw;
        if (cd > fontObj->max_x_size)
            fontObj->max_x_size = cd;
        if (ch > fontObj->y_size)
            fontObj->y_size = ch;
        if (cy > fontObj->y_size)
            fontObj->y_size = cy;
        if (cw != 0)
        {
            // packed bits
...
Font TFT_getFont(uint8_t font)
{
    ...
    else
    {
        if (font == DEJAVU18_FONT)
            fontObj.font = tft_Dejavu18;
        else if (font == DEJAVU24_FONT)
            fontObj.font = tft_Dejavu24;
        else if (font == UBUNTU16_FONT)
            fontObj.font = tft_Ubuntu16;
        else if (font == COMIC24_FONT)
            fontObj.font = tft_Comic24;
        else if (font == MINYA24_FONT)
            fontObj.font = tft_minya24;
        else if (font == TOONEY32_FONT)
            fontObj.font = tft_tooney32;
        else if (font == SMALL_FONT)
            fontObj.font = tft_SmallFont;
        else if (font == DEF_SMALL_FONT)
            fontObj.font = tft_def_small;
        else
            fontObj.font = tft_DefaultFont;

        fontObj.bitmap = 1;
        fontObj.x_size = fontObj.font[0];
        fontObj.y_size = fontObj.font[1];
        if (fontObj.x_size > 0)
        {
            fontObj.offset = fontObj.font[2];
            fontObj.numchars = fontObj.font[3];
            fontObj.size = fontObj.x_size * fontObj.y_size * fontObj.numchars;
        }
        else
        {
            fontObj.offset = 4;
            getMaxWidthHeightForFont(&fontObj);
        }
        //_testFont();
    }
...
//==============================================================================
void TFT_printWithColor(char *st, int x, int y, color_t fg, color_t bg)
{
    color_t lastFg = _fg;
    color_t lastBg = _bg;
    _fg = fg;
