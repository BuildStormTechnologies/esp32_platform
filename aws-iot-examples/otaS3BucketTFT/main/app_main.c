...
typedef enum
{
    ...
    STATE_SYS_OTA_INIT,
    STATE_SYS_OTA_BEGIN,
    STATE_SYS_OTA_IN_PROGRESS,
    STATE_SYS_OTA_SUCCESS,
    STATE_SYS_OTA_FAILED,
    STATE_SYS_OTA_REBOOT,
    STATE_SYS_MAX
} sysStates_et;
...
void app_statusUpdate()
{
    ...
    static uint8_t otaDispState = 0;
    static uint32_t rebootMsgTime = 0;
    ...
    if (WIFI_isConnected())
    {
       ...
       // awsState = get_tempAwsState();
        ...
        if(AWS_getState() == STATE_AWS_CONNECTED)
        {
            ...
            {              
              case STATE_OTA_INIT:
              case STATE_OTA_HTTP_CONNECT:
                systemStateTemp = STATE_SYS_OTA_INIT;
                if(otaDispState!=1)
                {
                    otaDispState = 1;
              ...
              case STATE_OTA_BEGIN:
                systemStateTemp = STATE_SYS_OTA_BEGIN;
                otaDispState = 2;                         
              break;
              
              case STATE_OTA_IN_PROGRESS:
                systemStateTemp = STATE_SYS_OTA_IN_PROGRESS;
                otaDispState = 3;
              break;
              
              case STATE_OTA_SUCCESS:
               otaDispState = 4;
               rebootMsgTime = millis()+2000;
               systemStateTemp = STATE_SYS_OTA_SUCCESS;
              break;     
              
              case STATE_OTA_FAILED:
                systemStateTemp = STATE_SYS_OTA_FAILED;
                otaDispState = 5;
              break;
        ...
        if((otaDispState >= 4) && (millis()>rebootMsgTime))
        {
            systemStateTemp = STATE_SYS_OTA_REBOOT;
            otaDispState = 6;
        }
    }
    else
    ...
    if (systemStateTemp != systemState)
    {
        ...
        if(systemState >= STATE_SYS_OTA_IDLE)
        {  
            DISP_otaScreen(otaDispState);
        }
        else
        {   
...
void app_task(void *param)
{
    ...
    while (1)
    {
        ...
        {
            if (AWS_isConnected() && (systemState < STATE_SYS_OTA_INIT))
            {
                // Publish Cyclic messages
                if (millis() > nextAwsPublishTime)
                {
                    nextAwsPublishTime = millis() + 2000;
                    awsAppMsg.payloadLen = sprintf(awsAppMsg.payload, "%s Hello AWS %d", APP_VERSION, pubCount++);
