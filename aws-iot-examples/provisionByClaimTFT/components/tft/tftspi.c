...
esp_err_t IRAM_ATTR wait_trans_finish(uint8_t free_line)
{
    ...
    if (_dma_sending)
    {
        ...
        if (disp_spi->host->dma_chan)
            spi_lobo_dmaworkaround_idle(disp_spi->host->dma_chan);
...
{
    if (!(disp_spi->cfg.flags & LB_SPI_DEVICE_HALFDUPLEX))
        return;
...
static void IRAM_ATTR _direct_send(color_t *color, uint32_t len, uint8_t rep)
{
    ...
    while (len)
    {
        ...
        if (rep == 0)
            cidx++; // if not repeating color, increment color buffer index
    }
    if (bits)
    {
        while (disp_spi->host->hw->cmd.usr)
...
int IRAM_ATTR read_data(int x1, int y1, int x2, int y2, int len, uint8_t *buf, uint8_t set_sp)
{
    ...
    if (set_sp)
    {
        if (disp_deselect() != ESP_OK)
            return -1;
        // Change spi clock if needed
        current_clock = spi_lobo_get_speed(disp_spi);
        if (max_rdclock < current_clock)
            spi_lobo_set_speed(disp_spi, max_rdclock);
    }
    ...
    if (set_sp)
    {
        ...
        if (max_rdclock < current_clock)
            spi_lobo_set_speed(disp_spi, current_clock);
    }
...
uint32_t find_rd_speed()
{
    ...
    for (uint32_t speed = 2000000; speed <= cur_speed; speed += 1000000)
    {
        ...
        if (ret == ESP_OK)
        {
            ...
            {
                if ((color_line[y].r & 0xFC) != (rdline[y].r & 0xFC))
                    line_check = 1;
                else if ((color_line[y].g & 0xFC) != (rdline[y].g & 0xFC))
                    line_check = 1;
                else if ((color_line[y].b & 0xFC) != (rdline[y].b & 0xFC))
                    line_check = 1;
                if (line_check)
                    break;
            }
